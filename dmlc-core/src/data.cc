...
Parser<IndexType> *
CreateCTRParser(const std::string& path,
                const std::map<std::string, std::string>& args,
                unsigned part_index,
                unsigned num_parts) {
  InputSplit* source = InputSplit::Create(
      path.c_str(), part_index, num_parts, "text");
  int nthreads = dmlc::GetEnv("MXNET_HDFS_NTHREAD", 1);
  ParserImpl<IndexType> *parser = new CTRParser<IndexType>(source, args, nthreads);
#if DMLC_ENABLE_STD_THREAD
  parser = new ThreadedParser<IndexType>(parser);
#endif
  return parser;
}
DMLC_REGISTER_PARAMETER(CTRParserParam);
...
DMLC_REGISTER_DATA_PARSER(uint32_t, ctr, data::CreateCTRParser<uint32_t>);
DMLC_REGISTER_DATA_PARSER(uint64_t, ctr, data::CreateCTRParser<uint64_t>);
